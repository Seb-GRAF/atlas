...
import { AtlasButton, GlassPanel, Icons } from '../components';

type ScanSourceState = 'done' | 'running' | 'queued' | 'error';
...
function rightLabel(source: ScanSource): { text: string; mono: boolean; color: string } {
...
export function ScanProgressCard({ done, total, sources, onCancel, onBackground }: ScanProgressCardProps) {
  ...
  return (
    <GlassPanel variant="card" style={cardStyle}>
      ...
      <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
        <AtlasButton variant="secondary" onClick={onCancel} style={{ flex: 1, padding: '8px 12px', fontSize: 12.5 }}>
          Annuler
        </AtlasButton>
        <AtlasButton variant="primary" onClick={onBackground} style={{ flex: 1, padding: '8px 12px', fontSize: 12.5 }}>
